        ...
        for sub in subscriptions:
            ...
            if now >= sub_created_at + timedelta(hours=1) and now < sub_end_date and traffic_used < 0.005:
                if not await notification_sent(self.db, user.id, sub.id, FunnelNotificationType.TRIAL_NOT_CONNECTED_1H):
                    text = (
                        ...
                        'Мы заметили, что вы <u>ещё не начали пользоваться</u> тестовым периодом.\n\n'
                        '• <i>Статус ключа: активен и готов к работе</i>\n'
                        '• <i>Инструкция: пошаговое подключение под ваше устройство</i>\n\n'
                        'Если возникли сложности с установкой или импортом ключа — напишите нам в поддержку, мы быстро поможем настроить!'
                    )
            ...
            if now >= sub_created_at + timedelta(hours=2) and now < sub_end_date and traffic_used >= 0.005:
                if not await notification_sent(self.db, user.id, sub.id, FunnelNotificationType.TRIAL_STARTED):
                    text = (
                        ...
                        'Прошло два часа с момента запуска пробного доступа.\n\n'
                        '• Скорость: <code>без ограничений</code>\n'
                        '• Доступные локации: <code>все серверы сети</code>\n\n'
                        'Если всё устраивает, вы можете <u>выбрать постоянный тариф заранее</u>, чтобы подключение не прерывалось.'
                    )
                    keyboard = InlineKeyboardMarkup(
                        inline_keyboard=[
            ...
            if now >= sub_created_at + timedelta(hours=24) and now < sub_end_date and traffic_used < 0.005:
                if not await notification_sent(self.db, user.id, sub.id, FunnelNotificationType.TRIAL_NOT_CONNECTED_24H):
                    text = (
                        ...
                        'Прошли сутки с момента активации, но вы пока <u>не подключились к сети</u>.\n\n'
                        '• Доступ: <code>бесплатный период активен</code>\n'
                        '• Если приложение не запускается — откройте инструкцию или напишите нам в поддержку, мы оперативно поможем.'
                    )
                    keyboard = InlineKeyboardMarkup(
                        inline_keyboard=[
                            [build_miniapp_or_callback_button(text='Инструкция', callback_data='menu_info', cabinet_path='/info')],
            ...
            if sub_end_date - now <= timedelta(hours=24) and now < sub_end_date:
                if not await notification_sent(self.db, user.id, sub.id, FunnelNotificationType.TRIAL_EXPIRING_24H):
                    text = (
                        '⏳ <b>Остался 1 день пробного доступа</b>\n\n'
                        'Завтра ваш бесплатный тестовый период <u>завершится</u>.\n\n'
                        '• Срок действия: <code>менее 24 часов</code>\n'
                        '• Чтобы сохранить стабильное подключение без повторной настройки — <u>выберите постоянный тариф</u> в меню.'
                    )
                    keyboard = InlineKeyboardMarkup(
                        inline_keyboard=[
            ...
            if now >= sub_end_date and now < sub_end_date + timedelta(hours=24):
                if not await notification_sent(self.db, user.id, sub.id, FunnelNotificationType.TRIAL_EXPIRED_SURVEY):
                    text = (
                        ...
                        'Срок действия пробного доступа истёк, и подключение <u>приостановлено</u>.\n\n'
                        '• <i>Все ваши настройки и ключи надёжно сохранены</i>\n'
                        '• <i>Для продолжения работы вы можете оформить подписку в меню в любое время</i>\n\n'
                        'Поделитесь вашим впечатлением, всё ли работало стабильно? Если возникли трудности — мы на связи в поддержке.'
                    )
                    keyboard = InlineKeyboardMarkup(
                        inline_keyboard=[
            ...
            if now >= sub_end_date + timedelta(hours=24) and now <= sub_end_date + timedelta(days=5):
                if not await notification_sent(self.db, user.id, sub.id, FunnelNotificationType.TRIAL_DISCOUNT_15):
                    ...
                    text = (
                        ...
                        'Мы подготовили для вас <u>персональное предложение</u> на любой тариф.\n\n'
                        '• Ваша скидка: <b>-15%</b>\n'
                        '• Срок действия: <code>48 часов</code>\n'
                        '• Расчёт: <code>скидка применится автоматически при переходе к оплате</code>'
                    )
                    keyboard = InlineKeyboardMarkup(
                        inline_keyboard=[
        ...
        for sub in subscriptions:
            ...
            if not await notification_sent(self.db, user.id, sub.id, FunnelNotificationType.DORMANT_14D):
                ...
                text = (
                    ...
                    'Мы провели оптимизацию серверной инфраструктуры и <u>значительно улучшили скорость</u> зарубежных сервисов и видео.\n\n'
                    '• Персональный бонус: <b>-20%</b>\n'
                    '• Срок действия: <code>72 часа</code>\n'
                    '• Приглашаем вас снова оценить качество работы сети!'
                )
                keyboard = InlineKeyboardMarkup(
                    inline_keyboard=[
        ...
        for sub in subscriptions:
            ...
            if not await notification_sent(self.db, user.id, sub.id, FunnelNotificationType.REFERRAL_NPS_5D):
                text = (
                    ...
                    'Вы пользуетесь нашей подпиской уже несколько дней.\n\n'
                    '• <u>Делитесь вашей ссылкой с друзьями</u> — друг получит приятный бонус, '
                    'а вы будете получать <b>процент с каждого его пополнения</b> прямо на баланс.\n'
                    '• Накопленный баланс можно использовать для <code>100% оплаты тарифов</code>.'
                )
                keyboard = InlineKeyboardMarkup(
                    inline_keyboard=[
        ...
        for tx in transactions:
            ...
            if not await notification_sent(self.db, user.id, last_sub_id, FunnelNotificationType.FAILED_PAYMENT_1H):
                text = (
                    ...
                    'Похоже, платёж <u>не прошёл до конца</u>.\n\n'
                    '• Статус транзакции: <code>ожидает подтверждения</code>\n'
                    '• Если списались средства или возникла ошибка платёжного шлюза — свяжитесь с поддержкой, мы оперативно поможем!'
                )
                keyboard = InlineKeyboardMarkup(
                    inline_keyboard=[
